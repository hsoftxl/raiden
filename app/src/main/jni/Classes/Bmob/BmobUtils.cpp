...
void RemoteLogin::run(const std::function<void(int)> &callback) {
    ...
    param.insert(std::map<string, Ref *>::value_type("objectid", __String::createWithFormat("%s",
                                                                                            objectid.c_str())));
    param.insert(std::map<string, Ref *>::value_type("signature", __String::createWithFormat("%s",
                                                                                             signature.c_str())));

    bcloud->execCloudCode("https://api.m.taobao.com/rest/api3.do?api=mtop.common.getTimestamp",
                          param, this);
}
...
void RemoteLogin::onCloudSuccess(const void *data) {
    ...
    if (reader.parse((const char *) data, root)) {
        if (!root["data"].isNull()) {
            if (!root["data"]["t"].isNull()) {
                const std::string &time = root["data"]["t"].asString();
                GameData::getInstance()->setValue(GAMEDATA::TIME, Utils::timestamp2int(time));
            }
        }
    }
...
RemoteRapidDistance::RemoteRapidDistance(int distance)
        : m_callback(nullptr), m_iDistance(distance) {
