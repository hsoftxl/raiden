...
extern "C"
{
	...
	void Java_org_cocos2dx_cpp_NativeInterface_winAdReward(JNIEnv *env, jobject thiz, jstring position, jint resultCode) {
        std::string pt = JniHelper::jstring2string(position);
        DEBUG_LOG("position = %s.", pt.c_str());
        MenuSettle::addMoneyByAd((int)resultCode);
	}
...
void UniBilling::showAd(const std::string& position)
{
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
	JniMethodInfo minfo;//定义Jni函数信息结构体
	//getStaticMethodInfo 次函数返回一个bool值表示是否找到此函数
	bool isHave = JniHelper::getStaticMethodInfo(minfo,
		"org/cocos2dx/cpp/Billing", "showAd", "(Ljava/lang/String;)V");

	if (!isHave)
	{
		DEBUG_LOG("jni: UniBilling::showAd() not exists");
	}
	else
	{
		DEBUG_LOG("jni:UniBilling::showAd() ok");
		//调用此函数
		minfo.env->CallStaticVoidMethod(minfo.classID, minfo.methodID,charTojstring(minfo.env, position.c_str()));
		DEBUG_LOG("jni-java: UniBilling::showAd finish");
		return;
	}
#endif
}
