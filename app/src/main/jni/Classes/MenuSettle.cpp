...
int MenuSettle::m_iMoneyAd = 0;
ui::Button* MenuSettle::m_pBtnContinue_ad(nullptr);
...
bool MenuSettle::init()
{
	...
	//控件
	FIND_UI_CONTROL_RE(ui::Button*, "Continue_Btn_ad", m_pBtnContinue_ad, m_pRootNode);
	...
	Utils::addButtonTouchEffect(m_pBtnContinue_ad, Utils::ButtonTouchEffect::NONE, [this](Ref* pSender, ui::Widget::TouchEventType type)
	{
		Sound::playSound(SOUND::ID_CLICK);
    ...
    m_pBtnContinue_ad->setVisible(true);
...
// TODO
void MenuSettle::addMoneyByAd(bool result)
{
		GameData::getInstance()->incValue(GAMEDATA::MONEY, m_iMoneyAd);
		m_pBtnContinue_ad->setVisible(false);
}

bool MenuSettle::showAd()
{
    BillingImpl::getInstance()->showAd("win_ad_money");
	return true;
}
