...
MenuDayGift::MenuDayGift()
        : m_pRootNode(nullptr), m_pBtnReceive(nullptr), m_pBtnVip(nullptr),
          m_pBtnVipContinue(nullptr), m_pBtnGiveup(nullptr), m_pScrollView(nullptr) {
}
...
time_t MenuDayGift::truncTime(time_t t) {
    time_t xx = t / secondInOneDay * secondInOneDay;
...
bool MenuDayGift::isDisplayDailyGift() {
    ...
    int lastLoginTime = MenuDayGift::truncTime(
            GameData::getInstance()->getValueToInt(GAMEDATA::DAY_REWARD_LAST_LOGIN));
    ...
    DEBUG_LOG(
            "displayDayGift tt: %d, timeNow = %d, lastLoginTime = %d, dayCnt: %d, secondInOneDay = %d",
            tt, timeNow, lastLoginTime, dayCnt, secondInOneDay);
    if (dayCnt >= 2) {
        DEBUG_LOG("displayDayGift 1 dayCnt = %d", dayCnt);
        continueDays = 1;
        MenuDayGift::setTimeVal(timeNow, continueDays);

        return true;
    } else if (1 == dayCnt) {
        DEBUG_LOG("displayDayGift 2 dayCnt = %d", dayCnt);
        continueDays++;
        MenuDayGift::setTimeVal(timeNow, continueDays);
        ...
        if (day > 0) {
            DEBUG_LOG("displayDayGift 3 dayCnt = %d", dayCnt);
            MenuDayGift::setTimeVal(timeNow, day);
            return true;
        } else {
            DEBUG_LOG("displayDayGift 4 dayCnt = %d", dayCnt);
            return false;
        }
    }

    DEBUG_LOG("displayDayGift 5 dayCnt = %d", dayCnt);
    return false;
}
...
bool MenuDayGift::init() {
    ...
    DEBUG_LOG("MenuDayGift::m_day %d", m_day);
    ...
    Utils::addButtonTouchEffect(m_pBtnReceive, Utils::ButtonTouchEffect::NONE,
                                [this](Ref *pSender, ui::Widget::TouchEventType type) {
                                    ...
                                    layer->setOnExitCallback([this]() {
                                        Director::getInstance()->replaceScene(
                                                TransitionCrossFade::create(.2f,
                                                                            MenuMainUI::scene()));
                                    });
                                    this->addChild(layer);
                                });

    Utils::addButtonTouchEffect(m_pBtnVip, Utils::ButtonTouchEffect::NONE,
                                [this](Ref *pSender, ui::Widget::TouchEventType type) {
                                    Sound::playSound(SOUND::ID_CLICK);
    ...
    Utils::addButtonTouchEffect(m_pBtnVipContinue, Utils::ButtonTouchEffect::NONE,
                                [this](Ref *pSender, ui::Widget::TouchEventType type) {
                                    Sound::playSound(SOUND::ID_CLICK);
    ...
    Utils::addButtonTouchEffect(m_pBtnGiveup, Utils::ButtonTouchEffect::NONE,
                                [this](Ref *pSender, ui::Widget::TouchEventType type) {
                                    Sound::playSound(SOUND::ID_CLICK);

                                    Director::getInstance()->replaceScene(
                                            TransitionCrossFade::create(.2f, MenuMainUI::scene()));
                                });
...
void MenuDayGift::SetDataVal() {
    ...
    if (vipTime) {
        m_prompt = GlobalData::getInstance()->getParameter(
                MenuDayGiftConstant::STRING_DAY_GIFT_PROMPT_VIP);
        iGiftRatio = VipConstant::DailyGiftRatio; //vip双倍奖励
    } else {
        m_prompt = GlobalData::getInstance()->getParameter(
                MenuDayGiftConstant::STRING_DAY_GIFT_PROMPT);
        iGiftRatio = 1;
    }
    ...
    for (auto p : array) {
        if (p->day == m_day) {
            ...
            if (!strcmp(p->gift.c_str(), EquipPiece::Aircraft7)) //战机7碎片
            {
                ...
                const std::vector<TEquipmentData *> &equipArray = GlobalData::getInstance()->getEquipmentArray(
                        EquipTypeAircraft);
    ...
    if (bAircraft7PieceFlag) {
        m_prompt.append(GlobalData::getInstance()->getParameter(
                MenuDayGiftConstant::STRING_GUIDE_PIECE_AIRCRAFT7_1));
    }
